...
class AiRecommendationClient
  ...
  class TimeoutError < Error
    def initialize
      super("AI recommendation generation timed out.", status: :gateway_timeout)
    end
  end

  class UnavailableError < Error
    def initialize
      super("AI recommendation service is unavailable.", status: :service_unavailable)
    end
  end

  class InvalidResponseError < Error
    def initialize
      super("AI recommendation service returned an invalid response.", status: :bad_gateway)
    end
  end
  ...
  def generate(payload)
    response = post_json(recommendation_uri, payload)
  ...
  def recommendation_uri
    base_url = @base_url.end_with?("/") ? @base_url : "#{@base_url}/"
    URI.join(base_url, "generate-recommendation")
  end
