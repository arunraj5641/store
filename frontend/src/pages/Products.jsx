const Products = () => {
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900/70 p-8">
      <p className="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-400">Products</p>
      <h1 className="mt-3 text-2xl font-semibold text-slate-100">Product catalog</h1>
      <p className="mt-2 text-slate-400">Placeholder page for product management UI.</p>
    </section>
  )
}

export default Products
